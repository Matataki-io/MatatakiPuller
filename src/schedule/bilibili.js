...
let getDynamicByID = async () => {
    const list = await Axios.get(`https://auth.matataki.io/api/user/info/bilibili`, { params: { apiToken: config.apiToken } })
    try {
...
let getDynamicByIDTest = async () => {
    const list = await Axios.get(`https://auth.matataki.io/apitest/user/info/bilibili`, { params: { apiToken: config.apiToken } })
    try {
        list.data.forEach(async item => {
